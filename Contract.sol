...
contract SimpleAuction {
    ...
    //Use mapping to avoid looping to determine actioneers in this list
    ...
    // Set to true when Auction Authority stops registration.
    // By default initialized to "false".
    ...
    //Register auctioneer if not in auctioneers list
    function registerAuctioneer() public{
        require(!registrationEnded, "Auction registration is closed");
        require(!containsAuctioneer(msg.sender),"Auctioneer already registered");
        setAuctioneer(msg.sender);
    }

    //Adds auctioneer to auctioneers list
    function setAuctioneer(address _auctioneer) private{
        auctioneers[_auctioneer]=true;
    }

    //Determines if auctioneer is in auctioneers list
    function containsAuctioneer(address _auctioneer) private view returns (bool){
        return auctioneers[_auctioneer];
    }

    function registerStop() public{
        require(!registrationEnded, "Registration already stopped");
